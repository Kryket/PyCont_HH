...
## ====================================
##             ~ANTHONY~
## MAKE SURE TO MESS WITH ALL THE LINES
## ====================================
"""

connor.set(tdata=[0,10])

connor.set(pars={
    'Iapp': 0., 't_on': 0, 't_off': 0,
    'gNa': 120., 'gK': 40.,
    'gA': 3., 'gNaP': 40, 'gK2': 20.,
    'gL': 0.3, 'EL': -65.
})
...
##=======================================================
##                          Start
##  NeurDyLib-GoldenSectionSearch.py specific code pt.2
##=======================================================
...
connor.pars['gNaP'] =  (((upper_gNaP - lower_gNaP) / phi) + lower_gNaP)
...
for i in range(5): 
    ...
    ax_v = fig.add_subplot(611)
    ax_na = fig.add_subplot(612)
    ax_k = fig.add_subplot(613)
    ax_A = fig.add_subplot(614)
    ax_NaP = fig.add_subplot(615)
    ax_K2 = fig.add_subplot(616)


    fig.suptitle('Steady State Solution', fontsize=14, fontweight='bold')

    ax_v.plot(pts['t'], pts['v'], 'b', label='v')
    ax_v.set_ylabel('V')
    ax_v.legend()

    ax_na.plot(pts['t'], pts['m'], 'g', label='m')
    ax_na.plot(pts['t'], pts['h'], 'r', label='h')
    ax_na.set_ylabel('I_Na')
    ax_na.legend()

    ax_k.plot(pts['t'], pts['n'], 'r', label='n')
    ax_k.set_ylabel('I_K')
    ax_k.legend()

    ax_A.plot(pts['t'], pts['B'], 'c', label='B')
    ax_A.plot(pts['t'], pts['A'], 'm', label='A')
    ax_A.set_ylabel('I_A')
    ax_A.legend()

    ax_NaP.plot(pts['t'], pts['hNaP'], 'y', label='hNaP')
    ax_NaP.set_ylabel('I_NaP')
    ax_NaP.legend()

    ax_K2.plot(pts['t'], pts['mK2'], 'k', label='mK2')
    ax_K2.set_ylabel('I_K2')
    ax_K2.legend()
...
##=======================================================
    ...
    plt.savefig('C:\Users\Smash\Pictures\(%.6f,%.6f).pdf' % (connor.pars['gNaP'],connor.pars['gK2']))

    print "========================\nDoes the graph resemble a Hopf or SNIC? \n========================"
    answer = raw_input()
    answer

    if answer == "Hopf":
        connor.pars['gNaP'] = (((connor.pars['gNaP'] - lower_gNaP) / phi) + lower_gNaP)
        print "\n Creating new graph where gNaP = %.6f \n" % (connor.pars['gNaP'])
        plt.close
    elif answer == "SNIC":
        connor.pars['gNaP'] = ((( upper_gNaP - connor.pars['gNaP']) / phi) + connor.pars['gNaP'])
        print "\n Creating new graph where gNaP = %.6f \n" % (connor.pars['gNaP'])
        plt.close
    else:
        print "\n You either broke the rules or you broke this project off the rails... \n  Whichever it is, congratulations! You are a monster."
        break
##=======================================================
##                          End
##  NeurDyLib-GoldenSectionSearch.py specific code pt.2
...
##======================================
##                 Start
##  PyCont_HH [v0.4].py specific code
##======================================
...
##Moves the sampling vertically[gNaP] or horizontally[gK2]##
